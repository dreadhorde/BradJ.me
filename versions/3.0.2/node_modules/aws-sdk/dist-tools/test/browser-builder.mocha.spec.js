(function() {
  var Builder, exec, expect, fs, helpers;
  ...
  describe('Builder', function() {
    describe('addServices', function() {
      var add, assertBundleFailed, assertServiceAdded, builder, code;
      builder = null;
      code = null;
      beforeEach(function() {
        return builder = new helpers.Builder;
      });
      add = function(services) {
        builder.addServices(services);
        return code = builder.serviceCode.join('\n');
      };
      assertServiceAdded = function(klass, version) {
        ...
        return expect(code).to.match(new RegExp('AWS\\.Service\\.defineServiceApi\\(AWS\\.' + klass + ', "' + version + '",'));
      };
      ...
      it('accepts "all" for all services', function() {
        add('all');
        return Object.keys(builder.buildStrategy.serviceClasses).forEach(function(s) {
          var name;
    ...
    return describe('build', function() {
      var buildBundle, bundleCache, data;
      bundleCache = {};
      data = null;
      beforeEach(function() {
        return data = '';
      ...
      buildBundle = function(services, opts, code, cb) {
        ...
        opts = opts || {};
        return new Builder(opts).addServices(services).build(function(err, d) {
          data = d;
      ...
      describe('cached writer', function() {
        it('writes core and services to disk', function(done) {
          var builder, root;
          ...
          return builder.addServices('s3').build(function() {
            try {
              return ['s3.js', 's3-latest.js', '_core.js'].forEach(function(item) {
                return expect(fs.existsSync(root + item)).to.equal(true, item + ' exists?');
              });
            } finally {
        ...
        return it('writes minified core and services to disk', function(done) {
          var builder, root;
          root = __dirname + '/tmpcache/';
          builder = new Builder({
            writeCache: true,
          ...
          return builder.addServices('s3').build(function() {
            try {
              return ['s3.min.js', 's3-latest.min.js', '_core.min.js'].forEach(function(item) {
                return expect(fs.existsSync(root + item)).to.equal(true, item + ' exists?');
              });
            } finally {
              exec('rm -rf ' + root, done);
      ...
      describe('cached loading', function() {
        var root;
        root = __dirname + '/tmpcache/';
        it('sets up cache', function(done) {
          var builder;
          builder = new Builder({
            writeCache: true,
            cacheRoot: root
          });
          return builder.addServices('s3').build(done);
        });
        it('loads from cache', function() {
          ...
          return builder.addServices('all').build(function(err, code) {
            return expect(code).to.contain('AWS.S3 = ');
          });
        });
        it('fails if cache item is missing', function() {
          var builder;
          builder = new Builder({
            cache: true,
            cacheRoot: root
          ...
          return expect(function() {
            return builder.addServices('kinesis');
          }).to["throw"]('Missing modules: kinesis');
        });
        it('fails to load from core if core is missing', function(done) {
          return exec('rm ' + root + '_core.js', function() {
            var builder;
            builder = new Builder({
              cache: true,
              cacheRoot: root
            });
            return builder.addServices('s3').build(function(err, data) {
        ...
        return it('removes cache', function(done) {
          return exec('rm -rf ' + root, done);
        });
      });
