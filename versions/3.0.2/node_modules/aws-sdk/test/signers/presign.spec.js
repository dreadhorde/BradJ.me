(function() {
  ...
  describe('AWS.Signers.Presign', function() {
    ...
    beforeEach(function() {
      return spyOn(AWS.util.date, 'getDate').andReturn(new Date(0));
    });
    it('presigns requests', function() {
      return cw.listMetrics().presign(function(err, url) {
        return expect(url).toEqual(resultUrl);
      });
    });
    it('presigns synchronously', function() {
      return expect(cw.listMetrics().presign()).toEqual(resultUrl);
    });
    it('allows specifying different expiry time', function() {
      return expect(cw.listMetrics().presign(900)).toMatch('X-Amz-Expires=900&');
    });
    it('limits expiry time to a week in SigV4', function() {
      return cw.listMetrics().presign(9999999, function(err) {
        ...
        return expect(err.message).toEqual('Presigning does not support expiry time greater than a week with SigV4 signing.');
      });
    });
    return it('only supports s3 or v4 signers', function() {
      return new AWS.EC2().describeInstances().presign(function(err) {
        expect(err.code).toEqual('UnsupportedSigner');
        return expect(err.message).toEqual('Presigning only supports S3 or SigV4 signing.');
      });
    });
  });
