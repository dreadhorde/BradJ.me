  ...
  describe('AWS.S3', function() {
    ...
    describe('building a request', function() {
      ...
      describe('adding Content-Type', function() {
        beforeEach(function() {
          return spyOn(AWS.util, 'isBrowser').andReturn(true);
        });
        it('adds default content-type when not supplied', function() {
          ...
          return expect(req.headers['Content-Type']).toEqual('application/octet-stream; charset=UTF-8');
        });
        it('adds charset to existing content-type if not supplied', function() {
          ...
          return expect(req.headers['Content-Type']).toEqual('text/html; charset=UTF-8');
        });
        ...
        return it('does not add charset to non-string data', function() {
          ...
          return expect(req.headers['Content-Type']).toEqual('image/png');
        });
      });
    ...
    describe('SSE support', function() {
      ...
      it('encodes SSECustomerKey and fills in MD5', function() {
        var req;
        req = s3.putObject({
          Bucket: 'bucket',
          Key: 'key',
        ...
        req.build();
        expect(req.httpRequest.headers['x-amz-server-side-encryption-customer-key']).toEqual('S0VZ');
        return expect(req.httpRequest.headers['x-amz-server-side-encryption-customer-key-MD5']).toEqual('TzFsSjRNSnJoM1o2UjFLaWR0NlZjQT09');
      });
      return it('encodes CopySourceSSECustomerKey and fills in MD5', function() {
        var req;
        req = s3.copyObject({
          Bucket: 'bucket',
          Key: 'key',
        ...
        req.build();
        expect(req.httpRequest.headers['x-amz-copy-source-server-side-encryption-customer-key']).toEqual('S0VZ');
        return expect(req.httpRequest.headers['x-amz-copy-source-server-side-encryption-customer-key-MD5']).toEqual('TzFsSjRNSnJoM1o2UjFLaWR0NlZjQT09');
      });
    });
    describe('retry behavior', function() {
      return it('retries RequestTimeout errors', function() {
        ...
        return s3.putObject(function(err, data) {
          return expect(this.retryCount).toEqual(s3.config.maxRetries);
        });
      });
    });
    ...
    describe('completeMultipartUpload', function() {
      it('returns data when the resp is 200 with valid response', function() {
        ...
        return s3.completeMultipartUpload(function(error, data) {
          ...
          return expect(this.requestId).toEqual('656c76696e6727732072657175657374');
        });
      });
    ...
    describe('getBucketLocation', function() {
      ...
      return it('parses the location constraint from the root xml', function() {
        ...
        return s3.getBucketLocation(function(error, data) {
          ...
          return expect(this.requestId).toEqual('abcxyz');
        });
      });
    });
    ...
    AWS.util.each(AWS.S3.prototype.computableChecksumOperations, function(operation) {
      return describe(operation, function() {
        return it('forces Content-MD5 header parameter', function() {
          ...
          return expect(req.httpRequest.headers['Content-MD5']).toEqual(hash);
        });
      });
    });
    describe('willComputeChecksums', function() {
      ...
      it('does not compute checksums for Stream objects', function() {
        ...
        return expect(req.build(function() {}).httpRequest.headers['Content-MD5']).toEqual(void 0);
      });
    ...
    return describe('getSignedUrl', function() {
      ...
      it('gets a signed URL for getObject', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/key?AWSAccessKeyId=akid&Expires=900&Signature=4mlYnRmz%2BBFEPrgYz5tXcl9Wc4w%3D&x-amz-security-token=session');
      });
      it('gets a signed URL with Expires time', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/key?AWSAccessKeyId=akid&Expires=60&Signature=kH2pMK%2Fgm7cCZKVG8GHVTRGXKzY%3D&x-amz-security-token=session');
      });
      ...
      it('generates the right URL with a custom endpoint', function() {
        ...
        return expect(url).toEqual('https://bucket.foo.bar.baz:555/prefix/key?AWSAccessKeyId=akid&Expires=60&Signature=zA6k0cQqDkTZgLamfoYLOd%2Bqfg8%3D&x-amz-security-token=session');
      });
      ...
      it('gets a signed URL for putObject with no body', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/key?AWSAccessKeyId=akid&Expires=900&Signature=J%2BnWZ0lPUfLV0kio8ONhJmAttGc%3D&x-amz-security-token=session');
      });
      it('gets a signed URL for putObject with special characters', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/%21%40%23%24%25%5E%26%2A%28%29%3B%27%3A%22%7B%7D%5B%5D%2C./%3F%60~?AWSAccessKeyId=akid&Expires=900&Signature=9nEltJACZKsriZqU2cmRel6g8LQ%3D&x-amz-security-token=session');
      });
      it('gets a signed URL for putObject with a body (and checksum)', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/key?AWSAccessKeyId=akid&Content-MD5=hBotaJrYa9FhFEdFPCLG%2FA%3D%3D&Expires=900&Signature=4ycA2tpHKxfFnNCdqnK1d5BG8gc%3D&x-amz-security-token=session');
      });
      it('gets a signed URL and appends to existing query parameters', function() {
        ...
        return expect(url).toEqual('https://bucket.s3.amazonaws.com/?AWSAccessKeyId=akid&Expires=900&Signature=8W3pwZPfgucCyPNg1MsoYq8h5zw%3D&prefix=prefix&x-amz-security-token=session');
      });
      ...
      return it('errors when expiry time is greater than a week out on SigV4', function() {
        ...
        runs(function() {
          return s3.getSignedUrl('getObject', params, function(e, d) {
            data = d;
            return err = e;
          });
        });
